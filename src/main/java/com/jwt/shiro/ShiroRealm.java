package com.jwt.shiro;


import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.jwt.constant.Constant;
import com.jwt.entity.JwtToken;
...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        if (principals != null) {
            userModel = (UserModel) principals.getPrimaryPrincipal();
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        // 校验token有效性
        UserModel loginUser = this.checkUserTokenIsEffect(token);
     ...
     * @param token
     */
    public UserModel checkUserTokenIsEffect(String token) throws AuthenticationException {
    ...
    public boolean jwtTokenRefresh(String token, String userName, String passWord) {
        String cacheToken = String.valueOf(redisUtil.get(Constant.PREFIX_USER_TOKEN + token));
        if (!StringUtils.isEmpty(cacheToken)) {
            // 校验token有效性
            if (!JwtUtil.verify(cacheToken, userName, passWord)) {
                String newAuthorization = JwtUtil.sign(userName, passWord);
                redisUtil.set(Constant.PREFIX_USER_TOKEN + token, newAuthorization);
                // 设置超时时间
                redisUtil.expire(Constant.PREFIX_USER_TOKEN + token, JwtUtil.EXPIRE_TIME / 1000);
            } else {
                redisUtil.set(Constant.PREFIX_USER_TOKEN + token, cacheToken);
                // 设置超时时间
                redisUtil.expire(Constant.PREFIX_USER_TOKEN + token, JwtUtil.EXPIRE_TIME / 1000);
            }
            return true;
